export const SKILL_LABELS={
  ...
  'general.smalltalk':'Small talk','general.work':'Anglais professionnel','general.pronunciation':'Compréhension orale naturelle'
};
...
export function recomputeSkills(questions,attempts){
  const qmap=new Map(questions.map(q=>[q.id,q]));const map=new Map();
  for(const a of attempts){const q=qmap.get(a.questionId);if(!q)continue;const keys=[partKey(q.part),...(q.skills||[])].filter(Boolean);
    for(const key of keys){const s=map.get(key)||{id:key,attempts:0,correct:0,totalMs:0,mastery:.5};s.attempts++;s.correct+=a.correct?1:0;s.totalMs+=a.timeMs||0;const speed=Math.max(.2,Math.min(1.2,(q.timeTargetSec||30)*1000/Math.max(a.timeMs||1000,1000)));const recent=(a.correct?1:0)*.78+speed*.22;s.mastery=s.mastery*.8+recent*.2;map.set(key,s);}
  }
...
export function pickAdaptive(questions,attempts,skills,count=12){
  const qmapAttempts=new Map();for(const a of attempts)qmapAttempts.set(a.questionId,(qmapAttempts.get(a.questionId)||0)+1);const smap=new Map(skills.map(s=>[s.id,s]));
  return [...questions].map(q=>{let weakness=.5;const pk=partKey(q.part);if(pk)weakness=1-(smap.get(pk)?.mastery??.5);for(const sk of q.skills||[])weakness=Math.max(weakness,1-(smap.get(sk)?.mastery??.5));const repeat=qmapAttempts.get(q.id)||0;const freshness=1/(1+repeat*.6);return{q,score:weakness*.7+freshness*.25+Math.random()*.05};}).sort((a,b)=>b.score-a.score).slice(0,count).map(x=>x.q);
}
export function estimatedScores(questions,attempts){
  const qmap=new Map(questions.map(q=>[q.id,q]));
  const toeic=[...attempts].filter(a=>{const q=qmap.get(a.questionId);return q&&Number.isInteger(q.part)&&q.part>=1&&q.part<=7&&q.domain!=='general';}).sort((a,b)=>String(b.createdAt).localeCompare(String(a.createdAt))).slice(0,200);
...
export function cefrEstimate(questions,attempts){
  const qmap=new Map(questions.map(q=>[q.id,q]));
  const order=['pre-a1','a1','a2','b1','b2','c1'];let highest=null;
  for(const lv of order){const arr=attempts.filter(a=>qmap.get(a.questionId)?.level===lv&&qmap.get(a.questionId)?.domain==='general');if(arr.length<2)continue;const acc=arr.filter(a=>a.correct).length/arr.length;if(acc>=.67)highest=lv;else break;}
  return highest;
}
