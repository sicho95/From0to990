...
function ctx(){
  const score=estimatedScores(state.questions,state.attempts),cefr=state.profile.cefrLevel||cefrEstimate(state.questions,state.attempts),stats=computeStats(),next=nextLessonId(state.profile,state.attempts),lq=lessonQuestions(next),done=new Set(state.attempts.filter(a=>a.correct).map(a=>a.questionId));
  return{score,cefr,stats,nextLesson:next,nextProgress:Math.round(lq.filter(q=>done.has(q.id)).length/Math.max(1,lq.length)*100),priorities:priorities(state.skills,3,{general:true}),gains:computeGains(),weekGoal:(state.profile.timePerDay||20)*5}
}
function draw(){
...
async function saveOnboarding(){state.profile=await saveProfile({...state.profile,displayName:document.getElementById('on-name').value.trim()||'Damien',targetScore:Number(document.getElementById('on-target').value||990),timePerDay:Number(document.getElementById('on-time').value||20),profileSetupComplete:true})}
